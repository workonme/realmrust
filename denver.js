...
window.server_2_title = "RealmRust X50";
...
const menuConfig = [
    ...
    { title: 'Банлист', path: '/page/banlist', icon: 'banlist' },
  	...
  	{ title: 'Статистика', path: '/page/stats', icon: 'stats' }
  ];
...
(function () {
  function AddMainbarSection() {
    ...
    function createMainbarSection() {
      ...
      const server2TitleEsc = String(
        typeof window.server_2_title === 'string' ? window.server_2_title : 'Сервер 2'
      )
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
      mainbarContent.innerHTML = `
		...
		<div class="mainbar_monitoring">
			<div class="monitoring_content">
				<div id="progress-42403" class="monitoring-progress">
				...
				<div class="monitoring_info">
					...
					<button type="button" class="monitoring_connect_btn" data="connect_1" onclick="copyConnect(this)">Коннект</button>
				</div>
			</div>
			<div class="monitoring_content">
				<div id="progress-42404" class="monitoring-progress">
					<svg viewBox="0 0 100 100">
						<circle cx="50" cy="50" r="40" />
						<circle class="progress-circle" cx="50" cy="50" r="40" />
					</svg>
				...
				<div class="monitoring_info">
					...
					<div id="server-42404"></div>
					<button type="button" class="monitoring_connect_btn" data="connect_2" onclick="copyConnect(this)">Коннект</button>
				</div>
			</div>
	</div>`;
        ...
        setTimeout(() => {
          if (data.result === 'success' && data.data && data.data.servers) {
            ...
            serverIDs.forEach(sID => {
              ...
              } else if (serverElement || progressElement) {
                if (serverElement) {
                  serverElement.innerHTML = '<span style="color: rgb(120 120 120);font-size:13px;">Нет в виджете мониторинга</span>';
                }
                if (progressElement) {
                  const progressCircle = progressElement.querySelector('.progress-circle');
                  if (progressCircle) {
